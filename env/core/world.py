...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            wall_positions = set(zip(*np.where(self.world == self.WALL)))
            all_positions = [(x, y) for x in range(1, self.size[0] - 1) for y in range(1, self.size[1] - 1)]
            available_positions = list(set(all_positions) - wall_positions)
            available_positions = sorted(available_positions, key=lambda x: (x[0], x[1]))
            start_position = (random.randrange(available_positions[0][0], available_positions[-1][0]),
                              random.randrange(available_positions[0][1], available_positions[-1][1]))
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.snake.blocks[0] in set(zip(*np.where(self.world == self.WALL))):
                self.snake.alive = False
            # Check if snake eats itself
            elif self.snake.blocks[0] in self.snake.blocks[1:]:
                self.snake.alive = False
            #  Check if snake eats the food
            if self.snake.blocks[0] == self.food_position:
                # Remove old food
